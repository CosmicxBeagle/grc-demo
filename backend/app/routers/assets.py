...
def create_asset(data: AssetCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return AssetService(db).create(data)


@router.patch("/{asset_id}", response_model=AssetOut)
def update_asset(asset_id: int, data: AssetUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
