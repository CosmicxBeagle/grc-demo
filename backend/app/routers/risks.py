...
def risks_for_control(control_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return RiskService(db).get_risks_for_control(control_id)


@router.get("", response_model=list[RiskOut])
def list_risks(db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def create_risk(data: RiskCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    return RiskService(db).create(data)


@router.patch("/{risk_id}", response_model=RiskOut)
def update_risk(risk_id: int, data: RiskUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def link_control(risk_id: int, data: RiskControlCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def unlink_control(risk_id: int, control_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
